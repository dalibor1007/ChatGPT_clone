...
const Chat = (props: any) => {
  ...
  const sendMessage = async (e: any) => {
    ...
    const { error } = await supabase.from('messages').insert([{ content: message, role: "user" }]);
    if (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const { error: aiError } = await supabase.from('messages').insert([{ content: data.message, role: "system" }]);
        if (aiError) {
